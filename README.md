...
需管理员权限读取 System 日志。推荐通过 Windows Terminal 提权配置文件启动：

1. 打开 Windows Terminal → 下拉菜单 → **MAGI Monitor**
2. 弹出 UAC 确认 → 点「是」
3. 脚本在提升权限的 WT 标签页中运行

配置文件已自动创建于 Windows Terminal 设置中（`elevate: true`）。
...
监控 System 日志中以下 Event ID，推送全屏阻塞式告警：

| Event ID | 含义 | 来源 |
|----------|------|------|
| 129 | 重置到设备（NVMe 磁盘错误） | 磁盘控制器 |
| 136 | 设备移除/添加 | 磁盘控制器 |
| 153 | IO 错误 | 磁盘控制器 |
| 7040 | 服务启动类型变更 | Service Control Manager |

- **轮询间隔**: 5 秒，后台线程
- **告警**: 全屏深红色界面（红色闪烁边框）、`MessageBeep` 声音、事件详情
- **关闭**: 按任意键或鼠标点击
- **依赖**: `pywin32` (`pip install pywin32`)
- **权限**: 需要管理员权限（通过 Windows Terminal 提权配置文件启动）
...
- TEMP: GPU 温度
- CODEC: 解码器/编码器活动指示（DECODING/ENCODING 5Hz 闪烁）
- FG: DLSS 帧生成指示（D3D Optical Flow Accelerator >0% 时 3Hz 闪烁）
- FAN: 风扇转速
